...
void solve()
{
   ...
   vi pr(n,0), pc(n,0);
    ...
    for(int i = 1; i< n; ++i){
        if(s[i] == 'N'){pr[i] = pr[i-1]-1;pc[i] = pc[i-1];}
        else if(s[i] == 'S'){pr[i] = pr[i-1]+1;pc[i] = pc[i-1];}
        else if(s[i] == 'W')pc[i] = pc[i-1]-1,pr[i] = pr[i-1];
        else pc[i] = pc[i-1]+1,pr[i] = pr[i-1];
    }
   ...
   for(int i = len; i < n; ++i){
        ...
        if(lst < 0){
            debug(i,pr[i],pc[i])
            if(pr[i] == r and pc[i] == c)ans[i] = 1;
        }
        else {
            int x = pr[i]-pr[lst];
            int y = pc[i]-pc[lst];
            if(x == r and y == c)ans[i] = 1;
        }
   }
