...
//   debug(k, pref[n-1]);
  ...
  for(int i = 0; i < n; ++i){
    int temp = 0;
    if(i == 0)temp = pref[n-1];
    else temp = pref[n-1] - pref[i-1];

    int req = k - temp;
    int idx = lower_bound(all(pref), req) - pref.begin();

    if(pref[idx] == req)return 1;

    idx = lower_bound(all(pref), req + pref[n-1]) - pref.begin();
    if(pref[idx] == req + pref[n-1])return 1;
  }
...
//     int req = k - temp;
//     if(req%pref[n-1] == 0)return 1;

//     if(st.find(req%pref[n-1]) != st.end())return 1;
//     st.insert(req%pref[n-1]);
signed main()
{
