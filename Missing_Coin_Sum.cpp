...
/*
  arr = [1 2 2 7 9]
  ans = 1 , i = 0;
  if(arr[i] > 1)then return ans;
  else ans += arr[i]

  now ans = 2, i = 1, arr[1]= 2
  if(2> ans)return ans because if it is greater than 2 that means we can not make 2 by sum beacause if it is 3 then we can make sums like 1,3,4 etc but we can't make 2 becase
*/
