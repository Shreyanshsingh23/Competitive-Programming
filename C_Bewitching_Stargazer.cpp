...
struct Segment {
    ll left, right;
    Segment(ll l, ll r) : left(l), right(r) {}
};
...
ll sol(ll n, ll k) {
    ...
    while (!st.empty()) {
        ...
        if (len % 2 == 1) {
            ans += m;
            if (l != r) {
                if (r - (m + 1) + 1 >= k) st.push(Segment(m + 1, r));
                if (m - 1 - l + 1 >= k) st.push(Segment(l, m - 1));
            }
        } else {
            if (r - (m + 1) + 1 >= k) st.push(Segment(m + 1, r));
            if (m - l + 1 >= k) st.push(Segment(l, m));
        }
    }
