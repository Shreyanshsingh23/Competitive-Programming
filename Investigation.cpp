...
void solve()
{
   ...
   while(!q.empty())
   {
        auto [curDist, cur, flights] = q.top();
        q.pop();
        if(cur == n)
        {   
            if(ans.empty() or curDist == ans.back().F)ans.pb({curDist,flights});
            else break;
        }

        for(auto [e, wt]: a[cur])
        {
            q.push({curDist + wt, e, flights+1});
        }
   }
   
   cout << ans[0].F << ' ' << sz(ans) << ' ' <<  ans[0].S << ' ' << ans.back().S << ln;
