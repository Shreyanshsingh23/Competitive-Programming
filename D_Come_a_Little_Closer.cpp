...
int area(v<pi>& a) {
    ...
    if((maxX == minX or minY == maxY)  and sz(a) == area)return area + 1;
    if(sz(a) == area){
        area += min((maxX - minX + 1), (maxY - minY + 1));
        return area;
    }
    return area;
}
...
void solve() {
    ...
    sort(sy.begin(), sy.end(), [](auto a, auto b) { 
        return a.second < b.second; 
    });

    set<int> st;
    for (int i = 0; i < min(2LL, n); ++i) {
        st.insert(find(a.begin(), a.end(), sx[i]) - a.begin());
        st.insert(find(a.begin(), a.end(), sx[n - 1 - i]) - a.begin());
        st.insert(find(a.begin(), a.end(), sy[i]) - a.begin());
        st.insert(find(a.begin(), a.end(), sy[n - 1 - i]) - a.begin());
    }
