...
void solve()
{
   int1(n)
    int sum=0;
    // if n is power of 2 such that 2^k = n
    // then answer is 2^(k+1) -1 
    // i.e. because each ith bit changes every 2^i th number
    // so we can simply take the answer as sum of count for every bit by doing 
