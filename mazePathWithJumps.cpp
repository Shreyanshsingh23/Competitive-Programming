...
v<string> rec(int a, int b){
    ...
    for(int ver = 1; ver <= n-a; ++ver){
        v<string> vpaths = rec(a+ver,b);
        for(auto e : vpaths){
            string temp = "v";
            char ch = ver+'0';
            temp = ch+temp;
    ...
    int d = 1;
    // while(b+d <= m and a+d <= n)
    for(int d = 1; d <= m-b and d <= n-a; ++d)
    {
        v<string> dpaths = rec(a+d,b+d);
        for(auto e : dpaths){
            string temp = "d";
            char ch = d+'0';
            temp = ch+temp;
