...
void solve()
{
   ...
   function<void(int,int)> f = [&] (int node, int prev)
   {
       if(color[node] == 0)
       {
           ...
           ans[node].F += b[node];
           ans[node].S -= b[node];
           ans[node].S = max(0ll,ans[node].S);
           t[node] = max(t[node],ans[node].F);
       }
       else
       {
            if(prev != -1)ans[node] = ans[prev];
            ans[node].S += b[node];
            ans[node].F -= b[node];
            ans[node].F = max(0ll, ans[node].F);
            t[node] = max(t[node], ans[node].S);
       }
        for(auto e: a[node])
        {
            if(e != prev)
