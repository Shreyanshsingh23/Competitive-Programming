...
void backtrack(vi& curr_seq, int pos, int n, int m, 
               v<vi>& sequences) {
    if (pos == n) {
    ...
    int start = (pos == 0) ? 1 : curr_seq[pos-1] + 10;
    int end = (pos == n-1) ? m : m - 10*(n-pos-1);
    
    for (int i = start; i <= end; i++) {
        curr_seq.push_back(i);
        backtrack(curr_seq, pos + 1, n, m, sequences);
        curr_seq.pop_back();
    }
}
...
void solve()
{
    ...
    backtrack(curr_seq, 0, n, m, sequences);
