...
struct segmentTree {
    ...
    void set(int i, int val, int x, int lx, int rx) {
        ...
        int mid = (lx + rx) >> 1;
        if(i < mid) {
            set(i, val, x * 2 + 1, lx, mid);
        } else {
            set(i, val, x * 2 + 2, mid, rx);
        }
    ...
    int query(int k, int l, int x, int lx, int rx) {
        ...
        int right = arr[x * 2 + 2];
        int ans = 1e9;
        if(left >= k) ans = min(ans, query(k, l, x * 2 + 1, lx, mid));
        if(ans == 1e9 and right >= k) ans = min(ans, query(k, l, x * 2 + 2, mid, rx));
        return ans;
    }
