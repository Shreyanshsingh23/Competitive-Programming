...
ll solve_test_case(int n, int k, vector<ll>& a, vector<ll>& b) {
    // Collect unique prices to test
    set<ll> prices;
    for (int i = 0; i < n; i++) {
        prices.insert(a[i]);
        prices.insert(b[i]);
    ...
    for (ll price : prices) {
        ...
        for (int i = 0; i < n; i++) {
            if (price <= a[i]) {
                positive.push_back(price);
            } else if (price <= b[i]) {
                negative.push_back(price);
            }
        }
        ...
        total_customers += positive.size();
        total_revenue += 0;
        for(int i = 0; i < positive.size(); i++) {
            total_revenue += positive[i];
        }

        // Add up to k negative reviews
        for (int i = 0; i < min(k, (int)negative.size()); i++) {
            total_customers++;
            total_revenue += negative[i];
        }

        // Update maximum revenue
