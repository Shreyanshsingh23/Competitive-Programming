...
int n,m,k;
int dp[2001][2001];
// f(i,prev,k_left) -> returns the number of ways to print [i...n] walls such that there are exactly k_left points where the color of walls differ from left wall provided that last color was prev
int f(int i, int k_left)
{   
    //base case
    if(i == n)
    {
    ...
    if(i == 1)
    {
        ans = modAdd(ans,modMul(m,f(i+1,k_left-1),MOD),MOD);
    }
    else
    {
        if(k_left > 0)
        ans = modAdd(ans,modMul(m,f(i+1,k_left-1),MOD),MOD);       
        ans = modAdd(ans,f(i+1,k_left-1),MOD); 

    }
    return dp[i][k_left] = ans;
}
