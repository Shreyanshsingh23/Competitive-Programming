...
// f(sum,found) -> How many ways can we construct a path of sum sum with the constraint that at least one edge ≥ d is used?
int f(int sum, bool found)
{
    if(sum == n and found == 1)return 1;
    if(sum > n)return 0;

    if(dp[sum][found] != -1)return dp[sum][found];
    ...
    for(int i = 1; i <= k; ++i){
        if(sum + i <= n){
            if(i >= d)ans = modAdd(ans,f(sum + i, 1), MOD);
            else ans = modAdd(ans,f(sum + i, found), MOD);
        }
    }

    return dp[sum][found] = ans;
}
