...
vi w,s;
int dp[101][1000][101];
int f(int i, int sum_left, int k_left)
{
    //pruning
    ...
    //cache check
    if(dp[i][sum_left][k_left] != -1)
    {
        return dp[i][sum_left][k_left];
    }    
    //transitions
    int ans = f(i+1, sum_left, k_left);

    if(sum_left - w[i] >= 0 and k_left > 0)
    {
        ans = max(ans,s[i] + f(i+1, sum_left - w[i], k_left - 1));
    }

    return dp[i][sum_left][k_left] =  ans;
}

void printset(int i, int sum_left, int k_left)
{
    if(i == n)
    {
        cout << ln;
    ...
    if(f(i+1,sum_left-w[i],k_left-1) + s[i] > f(i+1,sum_left,k_left))
    {
        cout << s[i] << ' ';
        printset(i+1,sum_left-w[i], k_left-1);
    }
    else 
    {
        printset(i+1,sum_left,k_left);
    }
}
void solve()
{   
