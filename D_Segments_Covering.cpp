...
int modPow(int a, int b) { int ans = 1; while (b) { if (b & 1) ans = (ans * a) % 998244353; b /= 2; a = (a * a) % 998244353; } return ans; }
int modInv(int n){return modPow(n, 998244353 - 2); }
...
void solve()
{
   int n, m;
    ...
    for (int i = 0; i < n; ++i) 
    {
        ...
        int con = (1 - prob + mod) % mod;
        int val = (prob * modInv(con)) % mod;
    ...
    for (int i = 1; i <= m; ++i) 
    {
        ...
        for (auto& seg : seg[i]) 
        {
            ...
            int t = (dp[l - 1] * val) % mod;
            dp[i] = (dp[i] + t) % mod;
        }
    }

    int ans = (dp[m] * tem) % mod;
    cout << ans << ln;
