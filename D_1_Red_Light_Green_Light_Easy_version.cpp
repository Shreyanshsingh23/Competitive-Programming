...
void solve() {
    ...
    for(int j = 0; j < q; j++) {
        ...
        v<char> visited(n * 2 * k, 0);
        auto idx = [&](int i, int dir, int tmod) { return i * 2 * k + dir * k + tmod; };
        auto it = lower_bound(all(p), aj);
        int i = it - p.begin();
        vi current(3);
        if(it != p.end() and *it == aj) {
            current = {i, 0, 0};
        } 
        else 
        {
        ...
        while(true) {
            int ci = current[0], cdir = current[1], ctmod = current[2];
            if(visited[idx(ci, cdir, ctmod)]) {
                cout << "NO" << ln;
                break;
            }
            visited[idx(ci, cdir, ctmod)] = 1;
            int final_dir = (ctmod == d[ci]) ? 1 - cdir : cdir;
            if(final_dir == 0) 
            {
                if(ci == n-1) {
                    cout << "YES" << ln;
