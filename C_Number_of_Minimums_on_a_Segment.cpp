...
struct segmentTree {
    ...
    void init(int n) {
        ...
        arr.assign(2 * size, {1ll, 1e10}); // {cnt, minValue}
    }
    ...
    void build(int x, int l, int r, vi& a) {
        ...
        auto left = arr[x * 2 + 1];
        auto right = arr[x * 2 + 2];

        if(left.second == right.second) {
            arr[x].first = left.first + right.first;
            arr[x].second = left.second;
    ...
    pi cnt(int x, int lx, int rx, int l, int r) {
        ...
        int mid = (lx + rx) >> 1;
        auto left = cnt(x * 2 + 1, lx, mid, l, r);
        auto right = cnt(x * 2 + 2, mid, rx, l, r);
