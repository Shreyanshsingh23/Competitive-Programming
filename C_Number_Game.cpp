...
{
    if(n == 1)return 0;
    if(n < N and dp[n] != -1)return dp[n];
    if(n&1){
        if(n < N)return dp[n] = 1;
        else return 1;
    }
    ...
    if(ans == 0 and f(n-1) == 0)ans = 1;

    return n < N? dp[n] = ans: ans;
...
{
   int1(n)
//    for(int i = 1; i <= 100; ++i)debug(i,f(i))
    return f(n);
}
...
signed main()
{
    ...
    {
      //  cout << "Case #" << i << ": "; 
       cout << (solve() ? "Ashishgup": "FastestFinger") << ln;
        // solve();
    }
    return 0;
}
