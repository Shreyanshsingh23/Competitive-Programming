...
void solve()
{
   ...
   for(int i = 1; i <= n and imp == false; ++i)
   {
        if(d[i] == 1)
        {
            cur++;
        ...
        while(cur + st.size() > a[i].S) // cur+st.size() = maximum height we could have gain if we convert -1 to 1, so we have to assure that this maximum height should not cross upper limit of each level
        {
            if(st.empty())
            {
                cout << -1 << ln;
