...
bool isAlt(string& s, int start, int end) {
...
bool check(string s) {
    ...
    // First check if already alternating
    if (isAlt(s, 0, n-1)) return true;
    ...
    for (int l = max(0ll, firstViolation-1); l <= firstViolation+1 && l < n; l++) {
        for (int r = max(l+1, lastViolation); r <= min(n-1, lastViolation+2); r++) {
            string temp = s;
            reverse(temp.begin() + l, temp.begin() + r + 1);
            
            int checkStart = max(0ll, l-1);
            int checkEnd = min(n-1, r+1);
            if (isAlt(temp, checkStart, checkEnd)) {
                if (checkStart > 0 && !isAlt(temp, 0, checkStart)) continue;
                if (checkEnd < n-1 && !isAlt(temp, checkEnd, n-1)) continue;
                return true;
            }
        }
    }
