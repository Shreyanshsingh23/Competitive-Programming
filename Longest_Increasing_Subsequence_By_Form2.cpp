...
// f(i) -> returns the Length of longest increasing subsequence ending at index 'i'
int f(int i)
{
    if(i == 0){
        return 1;
...
void solve()
{
   ...
   int mxx = f(0);
   for(int i = 1; i< n; ++i)mxx = max(mxx,f(i));


   cout << mxx << ln;
   
   for(int i = 0; i < n; ++i)if(dp[i] == mxx)printSequence(i);
